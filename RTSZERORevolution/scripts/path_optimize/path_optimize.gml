...
function path_optimize() {
	...
	var pathId = argument[0];
	var gridSize = argument[1] * .5;
	var obstacle = argument[2];

	for(var i = 0; i < path_get_number(pathId) - 2; i++) {
	    var px1 = path_get_point_x(pathId, i);
	    var py1 = path_get_point_y(pathId, i);
	    var px2 = path_get_point_x(pathId, i + 2);
	    var py2 = path_get_point_y(pathId, i + 2);
